    ...
    <form action="{{ route('admin.management.role.update', $role->id) }}" method="POST" class="space-y-8">
        ...
        <div class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <label for="perm_{{ $permission->id }}" class="relative glass-panel p-4 rounded-2xl flex items-center gap-4 border border-white/5 transition-all has-[:checked]:bg-primary/10 has-[:checked]:border-primary/40 overflow-hidden {{ $isSuperAdmin ? 'opacity-80' : 'cursor-pointer hover:bg-white/5' }}">
                    <!-- Checkbox Overlay -->
                    @if(!$isSuperAdmin)
                    ...
                    @else
                    <input type="checkbox" checked disabled class="permission-checkbox peer absolute inset-0 opacity-0 z-10 cursor-default">
