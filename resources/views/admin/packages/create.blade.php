...
@section('page_description', 'Gabungkan beberapa layanan menjadi satu paket bundle spesial.')

@section('content')
<div class="max-w-4xl">
    <form action="{{ route('admin.packages.store') }}" method="POST" class="space-y-6" id="packageForm">
        @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-4">
                    ...
                    <div class="space-y-1">
                        ...
                        <input type="text" name="name" required value="{{ old('name') }}" placeholder="Contoh: Paket Sultan MLBB"
                    ...
                    <div class="space-y-1">
                        ...
                        <textarea name="description" rows="3" placeholder="Jelaskan isi paket ini..."
                            class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm focus:ring-1 focus:ring-primary outline-none transition-all">{{ old('description') }}</textarea>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-1">
                            ...
                            <div class="relative">
                                ...
                                <input type="number" name="price" required value="{{ old('price', 0) }}"
                    ...
                    <div class="space-y-1">
                        ...
                        <select name="status" class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm focus:ring-1 focus:ring-primary outline-none transition-all appearance-none cursor-pointer">
                            <option value="Aktif" class="bg-slate-900" {{ old('status') == 'Aktif' ? 'selected' : '' }}>Aktif</option>
                            <option value="Nonaktif" class="bg-slate-900" {{ old('status') == 'Nonaktif' ? 'selected' : '' }}>Nonaktif</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="space-y-6">
                    <div class="space-y-4">
                        ...
                        <div id="selectedServices" class="space-y-2 min-h-[50px] p-4 bg-white/5 rounded-2xl border border-dashed border-white/10 flex flex-col items-center justify-center text-slate-500 text-[10px]">
...
<script>
                ...
                .then(data => {
                    ...
                    if (data.length === 0) {
                        ...
                        data.forEach(service => {
                            ...
                            div.className = `flex items-center justify-between p-3 rounded-xl bg-white/5 border border-transparent hover:border-white/10 hover:bg-white/10 cursor-pointer transition-all group ${isSelected ? 'opacity-50 cursor-not-allowed pointer-events-none' : ''}`;
                            div.innerHTML = `
    ...
    function addService(id, name, price) {
        ...
        if (selectedIds.size === 1) {
            ...
            selectedContainer.classList.remove('flex-col', 'items-center', 'justify-center', 'text-slate-500', 'border-dashed');
        }
    ...
    window.removeService = function(id) {
        ...
        if (selectedIds.size === 0) {
            selectedContainer.innerHTML = `
                ...
                Belum ada layanan yang dipilih
            `;
            selectedContainer.classList.add('flex-col', 'items-center', 'justify-center', 'text-slate-500', 'border-dashed');
        }
    }

    // Assign addService to window for onclick accessibility
    window.addService = addService;

    // Handle initial state (errors/old input)
    @if(old('services'))
        // Re-adding old selected services would require their names/prices.
        // For simplicity in create view, we usually let users re-pick if validation fails.
    @endif
</script>
@endpush
