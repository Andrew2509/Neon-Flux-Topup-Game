...
<div class="space-y-6">
    ...
    @if(isset($sliders) && $sliders->count() > 0)
        ...
        <div class="relative w-full aspect-21/9 rounded-2xl overflow-hidden glass-panel-mobile shadow-md mb-6 group">
            <div class="swiper tablet-hero-swiper h-full">
                <div class="swiper-wrapper">
                    ...
                    <div class="swiper-slide relative">
                        ...
                        <div class="absolute inset-y-0 left-0 p-6 z-20 flex flex-col justify-center items-start transition-transform duration-500 group-active:translate-x-2 pointer-events-none">
                            <div class="max-w-[85%] space-y-2 pointer-events-auto bg-[#0a1020]/80 backdrop-blur-lg p-6 rounded-2xl border border-white/10 shadow-neon-blue">
                                {{-- Tablet Tags --}}
                                @if($slider->tags)
                                <div class="flex flex-wrap items-center gap-1.5 mb-1">
                                    @foreach(explode(',', $slider->tags) as $index => $tag)
                                    <span class="px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-tighter border backdrop-blur-md shadow-lg
                                ...
                                <div>
                                    <h2 class="text-2xl font-display font-black tracking-tight text-white line-clamp-1 drop-shadow-[0_0_15px_rgba(255,255,255,0.4)] uppercase leading-none">
                                        {{ $slider->title }}
                                    </h2>
                                    @if($slider->subtitle)
                                    <p class="text-xs font-black text-primary tracking-wide line-clamp-1 uppercase opacity-90 mt-1">
                                        {{ $slider->subtitle }}
                                    </p>
                                    @endif
                                </div>
                                ...
                                <p class="text-white text-xs line-clamp-2 max-w-sm drop-shadow-md opacity-100 font-medium">
                                    {{ $slider->description }}
                                </p>
                                @endif
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                var slideCount = document.querySelectorAll('.tablet-hero-swiper .swiper-slide').length;
                new Swiper('.tablet-hero-swiper', {
                    loop: slideCount > 1,
                    autoplay: { delay: 4000 },
    ...
    @else
        <div class="relative w-full aspect-21/9 rounded-2xl overflow-hidden glass-panel-mobile shadow-md">
            ...
            <div class="relative z-10 h-full p-4 flex flex-col justify-center">
                <h2 class="text-xl font-display font-black tracking-tighter text-white leading-tight">
                    {{ get_setting('site_name', 'PrincePay Gaming') }}
                </h2>
                <p class="text-white/80 text-[10px] mt-1 max-w-[150px] italic">{{ get_setting('site_tagline', 'Otomatis 24 Jam.') }}</p>
                <div class="mt-2.5 flex gap-1.5">
                    ...
                    <div class="px-2 py-1 rounded-md bg-white/10 border border-white/20 text-white font-medium text-[8px] uppercase">Cashback 5%</div>
                </div>
            </div>
            <img src="https://placehold.co/800x350/1e293b/ccc?text={{ urlencode(get_setting('site_name', 'PrincePay')) }}" class="absolute top-0 right-0 w-1/2 h-full object-cover opacity-20 dark:opacity-40" style="mask-image: linear-gradient(to left, black, transparent);">
        </div>
    @endif
    ...
    <div id="category-tabs-container-mobile" class="flex items-center gap-2 overflow-x-auto no-scrollbar pb-2 -ms-3 px-3">
        ...
        <button data-group="{{ $key }}" class="category-tab-mobile px-4 py-2.5 rounded-xl {{ $loop->first ? 'bg-primary text-black font-extrabold shadow-neon-cyan' : 'bg-white/5 border border-white/10 text-white/70 font-bold' }} whitespace-nowrap text-[11px] flex items-center gap-1.5 transition-all">
    ...
    <section>
        ...
        <div id="game-grid-mobile" class="grid grid-cols-6 sm:grid-cols-8 md:grid-cols-10 gap-2">
            ...
            <a href="{{ route('topup.game', $item->slug) }}" 
               ...
               class="game-card-mobile glass-panel-mobile p-1 rounded-lg flex flex-col items-center gap-1 shadow-sm active:scale-95 transition-all relative">
                @if($item->has_active_flash_sale)
                <!-- Flash Sale Badge -->
                <div class="absolute top-1 left-1 z-10">
                    <span class="bg-linear-to-r from-secondary to-pink-500 text-white text-[6px] font-black px-1 py-0.5 rounded shadow-lg flex items-center gap-0.5">
                        <span class="material-icons-round text-[8px]">bolt</span>
                        SALE
                    </span>
                </div>
                @endif
                <div class="w-full aspect-square rounded-md bg-black/5 dark:bg-white/5 overflow-hidden">
                ...
                <h4 class="text-[8px] font-bold text-white/90 text-center line-clamp-1 leading-tight">{{ $item->name }}</h4>
            </a>
            @empty
