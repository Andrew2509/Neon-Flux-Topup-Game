...
@section('title', 'Topup Mobile Legends — Neon Flux')
...
<div class="space-y-5 pb-20">
    ...
    <div class="glass-panel-mobile dark:bg-white/5 p-4 rounded-2xl space-y-3">
        <div class="flex items-center gap-2">
            ...
            <h3 class="font-bold text-sm text-white">Masukkan Data Akun</h3>
        </div>
        ...
        <div class="grid @if($category->has_zone) @else grid-cols-1 @endif gap-2">
            ...
            <div class="col-span-2 space-y-1">
                ...
                <input type="text" name="zone_id" id="zone_id_input" required placeholder="{{ $category->zone_placeholder ?? 'Zone ID' }}"
                    class="w-full rounded-xl bg-white/5 border border-white/10 px-3 py-2.5 text-xs focus:ring-primary focus:border-primary text-white font-mono shadow-sm">
            </div>
            @endif
        </div>
    ...
    <div id="nominal-section" class="glass-panel-mobile dark:bg-white/5 p-4 rounded-2xl space-y-3 transition-all duration-300">
        ...
        <div class="grid grid-cols-6 sm:grid-cols-8 gap-1.5" id="services-grid">
            ...
            <div class="relative group active:scale-95 transition-transform service-item" data-jenis="{{ $s->product_jenis_id }}">
                ...
                <div class="absolute -top-1 -right-1 w-4 h-4 bg-primary rounded-full flex items-center justify-center text-white scale-0 peer-checked:scale-100 transition-all duration-300 z-20 border border-white dark:border-slate-900 shadow-md overflow-hidden">
        ...
        <div class="space-y-4">
             ...
             <div class="space-y-2">
                ...
                <div class="relative group active:scale-[0.98] transition-all">
                    ...
                    <label for="p-{{ $p->id }}" class="flex items-center justify-between p-2 rounded-lg bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 peer-checked:border-primary peer-checked:bg-primary/5 transition-all">
                        <div class="flex items-center gap-2">
                            <div class="w-7 h-4 bg-white rounded-sm overflow-hidden flex items-center justify-center">
                                @if($p->image)
                                    <img src="{{ asset($p->image) }}" class="w-full h-full object-contain">
                                @else
                                    <span class="text-[6px] font-bold text-black">{{ strtoupper(substr($p->name, 0, 3)) }}</span>
                                @endif
                            </div>
...
<div class="fixed bottom-[52px] inset-x-0 z-40 bg-background-dark/95 backdrop-blur-xl border-t border-white/10 p-3 pb-safe flex items-center justify-center shadow-[0_-8px_30px_rgba(0,0,0,0.5)]">
    <div class="w-full max-w-5xl flex items-center justify-between">
        <div class="flex flex-col min-w-0">
            ...
            <span id="summary-player-name" data-sticky-summary="1" class="js-summary-player-name hidden text-[8px] text-white/60 font-medium max-w-[46vw] truncate leading-tight text-left"></span>
