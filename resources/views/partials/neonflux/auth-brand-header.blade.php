...
<div class="flex items-center gap-3 {{ $marginBottom }}">
    @if ($logo = get_image_url('site_logo'))
        <img src="{{ $logo }}" alt="{{ get_setting('site_name', 'NEON FLUX') }}" class="h-11 w-auto max-w-[220px] object-contain object-left shrink-0">
    @else
        <div class="size-9 bg-primary rounded-lg flex items-center justify-center text-white shadow-lg shadow-primary/20 shrink-0">
