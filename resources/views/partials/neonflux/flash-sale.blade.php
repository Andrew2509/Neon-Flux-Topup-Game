...
<!-- Flash Sale Section -->
<div class="mb-8 w-full glass-premium rounded-2xl overflow-hidden border border-white/20 shadow-xl relative">
    ...
    <div class="relative overflow-hidden p-2 md:p-3 flex items-center justify-between border-b border-white/10 gap-3 bg-white/5">
        ...
        <div class="flex items-center gap-2 relative z-10 w-full md:w-auto justify-start">
            ...
            <div class="flex flex-col">
                <h2 class="text-base md:text-lg font-display font-black text-white italic tracking-tighter uppercase leading-none text-glow">Flash Sale</h2>
                <span class="text-[7px] md:text-[8px] text-primary font-bold tracking-widest uppercase mt-0.5 opacity-80">Limit Edition</span>
            </div>
            
            <!-- Countdown Timer -->
            <div id="flash-sale-timer" class="ml-1 bg-white/10 backdrop-blur-xl px-2 py-0.5 rounded border border-white/20 font-mono text-xs md:text-sm font-black text-white shadow-[inset_0_0_8px_rgba(255,255,255,0.1)] min-w-[70px] text-center">
                00:00:00
            </div>
        </div>
        ...
        <div class="flex items-center justify-end gap-2 relative z-10">
            <!-- Live Status Badge -->
            <div class="flex items-center gap-1.5 bg-green-500/10 text-green-400 px-2 py-1 rounded-full border border-green-500/20 text-[8px] font-black uppercase tracking-widest">
            ...
            <!-- View All Button -->
            <a href="{{ route('flash-sale') }}" class="group flex items-center gap-1 bg-white/5 hover:bg-primary transition-all duration-300 px-2 py-1 rounded-full border border-white/10 text-[8px] font-black uppercase tracking-widest text-white/60 hover:text-slate-900 shadow-sm">
                <span class="hidden md:inline">View All</span>
                <span class="md:hidden">All</span>
                <span class="material-icons-round text-[10px] group-hover:translate-x-0.5 transition-transform">arrow_forward</span>
            </a>
        </div>
    </div>

    <!-- Flash Sale Items Container (Horizontal Scroll) -->
    <div class="p-2 overflow-hidden bg-transparent">
        <div class="flex flex-nowrap gap-2 overflow-x-auto pb-2 -mb-2 scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent scroll-smooth">
            ...
            @endphp
            <div class="flex-none w-[150px] md:w-[220px] group cursor-pointer" onclick="selectFromFlashSale('{{ $item->product_code }}', '{{ $item->category->slug }}')">
                <div class="glass-card-premium rounded-lg p-2 h-full relative overflow-hidden animate-shine border-white/5">
                    
                    <div class="flex gap-2 items-start text-left">
                        <!-- Thumbnail Area -->
                        <div class="relative w-12 h-12 md:w-14 md:h-14 flex-none rounded-lg overflow-hidden border border-white/10 shadow-lg group-hover:scale-105 transition-transform duration-500">
                            <img src="{{ $item->category->icon }}" alt="{{ $item->name }}" class="w-full h-full object-cover">
                            <!-- Discount Badge -->
                            <div class="absolute top-0 right-0 bg-primary text-[8px] md:text-[10px] font-black text-slate-900 px-1.5 py-0.5 rounded-bl-lg shadow-lg">
                                -{{ $percent }}%
                            </div>
                        </div>
                        ...
                        <div class="flex-1 min-w-0">
                            <h4 class="text-[7px] md:text-[8px] text-primary/80 font-bold truncate uppercase tracking-wider">{{ $item->category->name }}</h4>
                            <h3 class="text-[10px] md:text-xs font-black text-white leading-tight line-clamp-1 group-hover:text-primary transition-colors">{{ $item->name }}</h3>
                            
                            <div class="mt-0.5">
                                <span class="text-[8px] text-white/40 line-through">Rp {{ number_format($item->price, 0, ',', '.') }}</span>
                                <div class="text-sm md:text-base font-black text-white flex items-center gap-0.5">
                                    <span class="text-white text-[8px]">Rp</span>
