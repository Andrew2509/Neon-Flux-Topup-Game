...
<div class="w-full mb-10">
    ...
    <a href="{{ route('topup.game', $game->slug) }}" class="flex flex-col items-center glass-panel p-1.5 sm:p-2 lg:p-1.5 rounded-xl sm:rounded-2xl lg:rounded-xl card-hover transition-all cursor-pointer group relative overflow-hidden shadow-sm hover:shadow-xl dark:shadow-none">
        <div class="relative w-full aspect-square rounded-lg sm:rounded-xl lg:rounded-lg overflow-hidden mb-1.5 sm:mb-2 lg:mb-1 max-h-[5.5rem] sm:max-h-[6rem] md:max-h-[6.5rem] lg:max-h-[3.75rem]">
        ...
        <h3 class="text-[10px] sm:text-[11px] md:text-xs lg:text-[9px] font-bold text-slate-900 dark:text-white truncate w-full text-center group-hover:text-primary transition-colors leading-tight">{{ $game->name }}</h3>
        <p class="text-[8px] sm:text-[9px] lg:text-[7px] text-slate-500 dark:text-gray-500 mt-0.5 lg:mt-0 text-center transition-colors line-clamp-2 leading-snug">{{ $game->type }}</p>
    </a>
    @empty
