...
<head>
    ...
    <title>Reset Kata Sandi - PrincePayGaming</title>
...
<body class="bg-[#0a0a0a] min-h-screen flex items-center justify-center p-4 sm:p-8 selection:bg-primary/30 selection:text-white">
    ...
    <div class="flex flex-col lg:flex-row w-full max-w-[1200px] bg-black rounded-3xl overflow-hidden shadow-2xl border border-white/10 relative my-8">
        ...
        <div class="w-full lg:w-1/2 flex flex-col p-6 sm:p-8 lg:p-10 xl:p-12 justify-center bg-black min-h-[600px]">
            ...
            <!-- Title -->
            <div class="mb-8">
                <h1 class="text-white text-[2.25rem] font-black leading-none tracking-tight mb-3">Atur Ulang Kata Sandi</h1>
                <p class="text-zinc-400 text-base font-medium">Nomor Anda telah diverifikasi. Silakan masukkan kata sandi baru Anda di bawah ini.</p>
            </div>
            ...
            <!-- Form -->
            <form action="{{ route('password.reset.phone') }}" method="POST" class="space-y-6">
                @csrf
                <div>
                    <label class="block text-zinc-300 text-sm font-bold mb-2.5" for="password">Kata Sandi Baru</label>
                    <input class="w-full h-14 px-4 rounded-xl border @error('password') border-primary @else @enderror bg-zinc-900/50 text-white placeholder:text-zinc-600 focus:ring-2 focus:ring-primary focus:border-transparent transition-all outline-none font-medium" id="password" name="password" placeholder="Minimal 8 karakter" type="password" required autofocus />
                </div>
                <div>
                    <label class="block text-zinc-300 text-sm font-bold mb-2.5" for="password_confirmation">Konfirmasi Kata Sandi Baru</label>
